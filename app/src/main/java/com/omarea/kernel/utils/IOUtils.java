...
public class IOUtils {
    ...
    public static String[] getAvailableIOScheduler() {
        ...
        } else if (new File(Constants.available_schedulers_ufs).exists()) {
            /*
             * Some devices don't have mmcblk0 block device so we instead use
             * mtdblock0 to read the available schedulers
           */
            schedulerPath = Constants.available_schedulers_ufs;
    ...
    public static String getReadAhead() {
        ...
        if (res == null) {
            File device = new File(Constants.available_blockdevices + "sda");
            if (device.exists()) {
                device = new File(Constants.available_blockdevices + "sda/queue/read_ahead_kb");
                res = SysUtils.readOutputFromFile(device.getAbsolutePath());
            }
        }
        return res;
    ...
    public static void setDiskScheduler(String ioScheduler, Context context) {
        ...
        if (ioScheduler != null) {
            File[] devices = new File(Constants.available_blockdevices.substring(0, Constants.available_blockdevices.length()-1)).listFiles();
            if (devices == null) {
                String msg = context.getString(R.string.selinux_limit, getCurrentIOScheduler());
    ...
    public static void setReadAhead(String readAhead, Context ctx) {
        ...
        if (readAhead != null) {
            ...
            block = new File(Constants.available_blockdevices + "sda/queue/read_ahead_kb");
            if (block.exists()) {
                mCommands.add("chmod 0644 " + block.getAbsolutePath());
                mCommands.add("echo " + readAhead + " > " + block.getAbsolutePath());
            }
